...
./bin/loadmill --test-plan ${TEST_PLAN_ID} -j --junit-report-path="./test/tmp/cli" -m --mochawesome-report-path="./test/tmp/cli" --fetch-flow-runs -w -v -t ${TOKEN} --report --colors --additional-description 'loadmill-node-cli'